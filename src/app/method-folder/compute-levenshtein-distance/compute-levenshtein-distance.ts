export function computeLevenshteinDistance(leftString: string, rightString: string): number {
  ...
  for (let shorterStringIndex = 0; shorterStringIndex < shorterString.length + 1; shorterStringIndex++) {
  ...
  for (let longerStringIndex = 1; longerStringIndex < longerString.length + 1; longerStringIndex++) {
    ...
    for (let shorterStringIndex = 1; shorterStringIndex < shorterString.length + 1; shorterStringIndex++) {
